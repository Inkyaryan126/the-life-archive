...
-- RLS & Security: Private, Archive Owner-only access
...
create policy "Owners can view final wishes"
  ...
  using (auth.uid() = user_id or exists (
    select 1 from public.archives a where a.id = final_wishes.archive_id and a.owner_id = auth.uid()
  ));
...
create policy "Owners can view final wish songs"
  ...
  using (exists (
    select 1 from public.archives a where a.id = final_wish_songs.archive_id and a.owner_id = auth.uid()
  ));
