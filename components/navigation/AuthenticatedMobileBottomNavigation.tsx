...
export type AuthenticatedMobileBottomNavigationProps = {
...
export function AuthenticatedMobileBottomNavigation({
  activeArchiveSlug,
  hide = false
}: AuthenticatedMobileBottomNavigationProps) {
  ...
  const isMemoriesActive =
    pathname.startsWith("/archive/") && pathname.includes("/memories");
  const isAddMemoryActive = pathname.includes("/add-memory");
  const isTimeCapsulesActive = pathname.startsWith("/dashboard/time-capsules");
  const isMemberCardActive = pathname === "/member-card";
  ...
  const navItems = [
    ...
    },
    {
      ...
      icon: BookOpenIcon,
      isCenter: false,
      isActive: isMemoriesActive,
      disabled: !activeArchiveSlug
    },
    {
      label: "ADD MEMORY",
      href: addMemoryHref,
  ...
  return (
    <nav
      aria-label="Authenticated Mobile Bottom Navigation"
